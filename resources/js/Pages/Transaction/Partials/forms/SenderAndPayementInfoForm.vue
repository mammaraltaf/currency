<script setup>
    ...
    } from "@/helpers/currencies_countries";
    import {
        currencies
    ...
    const reciver_country = usePage().props.receivingCountries;
    ...
    const convertCurrencyByCountryCode = async () => {
        ...
        const selectedCountry = props.receivingCountries.find((country) => {
            return country.code_iso_2 === user.receiver_country || country.code === user
                .receiver_country
        });
    ...
    const verifyUserInformation = async (e) => {
        ...
        try {
            ...
            if (res.data.status === 'success') router.get('/verify-contacts?source=' + (props.transactionId ??
                'direct'));
        } catch (errors) {
...
<template>
    <div class="sender-info-form-wrapper">
        ...
        <NewTextInput v-model="user.first_name" :errors="api.errors.value?.first_name" label="First Name"
            placeholder="John" required title="first_name" class="sender-input" />
        ...
        <NewTextInput v-model="user.email" :errors="api.errors.value?.email" label="Email"
            placeholder="john@example.com" required title="email" class="sender-input" />

        <InternationalTelInput :errors="api.errors.value?.phone" :required="true" label="Phone Number"
            title="phone" class="sender-input">
            <vue-tel-input id="exampleFormControlInput1" ref="input" v-model="user.phone"
                :class="{ 'error-border': api.errors.value?.phone?.length > 0 }"
                :defaultCountry="geoLocationDetails?.country_code" :inputOptions="{ 'placeholder': '123456789' }"
                ...
                mode="international" @country-changed="phoneCountryChanged">
        ...
        <!-- start -->
        <NewSelectInput class="fifty-form-input" style="width: 100%" v-model="user.receiver_country"
            :errors="api.errors.value?.receiver_country" :options="countries" label="Country" placeholder="Country"
            disabled required title="country" type="text" @update:modelValue="getBankList" />
        <RadioButton v-model="user.type" :customStyle="{ width: 'inherit' }" :title="'Type'"
            :label="'Type'" :options="radioOptions" @update:modelValue="updateOptions" />
        <div class="receiver-info-form-wrapper">
            ...
            <NewTextInput class="fifty-form-input" v-model="user.receiver_last_name"
                ...
                title="last_name" />
            <NewTextInput class="fifty-form-input" v-if="user.type != 'bank'" v-model="user.receiver_email"
                :errors="api.errors.value?.receiver_email" label="Email" placeholder="john@example.com"
                title="email" />

            <InternationalTelInput v-if="user.type != 'bank'" :errors="api.errors.value?.receiver_phone"
                :required="true" label="Phone Number" title="phone" class="fifty-form-input">
                <vue-tel-input id="exampleFormControlInput1" ref="input" v-model="user.receiver_phone"
                    ...
                    :inputOptions="{ 'placeholder': '123456789' }" :defaultCountry="user.receiver_country"
                    :onlyCountries="isTransactionPreset ? [user.receiver_country] : []"
                    class="form-control block w-full px-0 py-0 text-lg font-normal h-10
              text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition
              ease-linear m-0
              focus:text-gray-700 focus:bg-white focus:border-indigo-300 focus:outline-none"
                    mode="international" @country-changed="receiverPhoneCountryChanged">
                </vue-tel-input>
            </InternationalTelInput>



            <NewSelectInput v-if="user.type == 'bank'" class="fifty-form-input" style="width: 100%"
                v-model="user.receiver_bank_id" :errors="api.errors.value?.receiver_bank_id" :options="bankOptions"
                label="International Bank" placeholder="International Bank" required title="bank" type="text" />
