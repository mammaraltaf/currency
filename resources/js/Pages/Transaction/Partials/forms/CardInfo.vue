...
watch(expiryYear, (newYear) => {
    ...
    if (newYear == currentYear) {
        ...
        for (let i = 12; i > 12 - upcomingMonths.length; i--) {
            j++;
            months.push({
                label: upcomingMonths[j - 1],
                value: currentMonth + j - 1,
            });
        }
    } else {
            ...
            <template #content>
                <div class="d-lg-flex d-md-flex d-sm-block">
                    <NewTextInput
                        ...
                        :errors="api.errors.value?.card_number"
                        :min="16"
                        label="Card Number"
                    ...
                    />
                    <span class="text-danger" v-if="cardValidation">{{
                        cardValidationError
                    }}</span>
                    <NewSelectInput
                        v-model="expiryYear"
                        :errors="api.errors.value?.expiry_year"
                        :options="expiryYears"
                        label="Expiry Year"
                        placeholder="Expiry Year of your payment card"
                        required
                        title="country"
                        class="fifty-form-input"
                        type="text"
                ...
                <NewSelectInput
                    v-model="user.expiry_month"
                    :errors="api.errors.value?.expiryMonth"
                    :options="expiryMonths"
                    label="Expiry Month"
                    placeholder="Expiry Month of your payment card"
                    required
                    title="expiry month"
                    class="fifty-form-input"
                    type="text"
